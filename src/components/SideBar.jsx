...
const SideBar = () => {
  ...
  const filteredUsers = debouncedInput ? users.filter((user) => user.fullName.toLowerCase().includes(debouncedInput.toLowerCase())) : users
    ...
    <div className={`bg-[#8185B2]/10 h-full p-5 rounded-r-xl overflow-y-scroll text-white ${selectedUser ? 'max-md:hidden' : ''}`}>
      <div className='pb-5'>
        <div className='flex justify-between items-center'>
          <img src={assets.logo40} alt="logo" className='max-w-40' />
          <div className='relative py-2' ref={dropdownRef}>
            ...
            />
            <div
              className={`absolute top-full right-0 z-20 w-32 p-5 rounded-md bg-[#282142] border border-gray-600 text-gray-100 origin-top-right transition-all duration-200 ease-out ${
                isOpen
          ...
          <div key={index}
            ...
            <div className='flex flex-col leading-5'>
              ...
              {typingUsers[user._id] ? (
                <span className='text-violet-300 text-xs italic animate-pulse'>typing...</span>
              ) : (
